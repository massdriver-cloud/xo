...
func TestHydrate(t *testing.T) {
	cases := []TestCase{
		...
		// },
		{
			...
			Expected: bundles.OrderedJSON([]bundles.OrderedJSONElement{
				{Key: "list", Value: bundles.OrderedJSON([]bundles.OrderedJSONElement{
					{Key: "string", Value: bundles.OrderedJSON([]bundles.OrderedJSONElement{
						{Key: "id", Value: "fake-schema-id"},
					})},
				})},
			}),
	...
	for _, test := range cases {
		t.Run(test.Name, func(t *testing.T) {
			...
			got := bundles.OrderedJSON(hydrated.([]bundles.OrderedJSONElement))

			// if !got.EqualSansIndex(test.Expected) {
